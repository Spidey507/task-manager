    ...
    <div class="bg-gray-700 py-10 shadow-md rounded flex flex-col w-1/3 items-center">
      ...
      <form class="flex-col content-center items-center justify-center">
        <div class="form-group">
          ...
          <input v-model="editedTask.title" type="text" required
            class="flex mb-4 p-2 border rounded focus:outline-none focus:border-blue-500 text-center" />
        </div>

        <div class="form-group">
          <label for="description" class="flex w-full justify-center text-white mb-2">Description</label>
          <input v-model="editedTask.description" type="text" required
            class="flex mb-4 p-2 border rounded focus:outline-none focus:border-blue-500 text-center" />
        </div>

        <div class="form-group">
          <label for="dueDate" class="flex w-full justify-center text-white mb-2">Due Date</label>
          <input v-model="editedTask.dueDate" type="date" required
            class="flex mb-4 p-2 border rounded focus:outline-none focus:border-blue-500 text-center w-full" />
        </div>
        ...
        <div class="button-group mt-4 flex justify-between">
          <button @click="updateTask" class="bg-blue-500 hover:bg-blue-600 text-white p-3 rounded flex-grow">Update
            Task</button>
          <button @click="cancelEdit"
            class="bg-red-500 hover:bg-red-600 text-white p-3 rounded flex-grow ml-2">Close</button>
        </div>
      </form>
    </div>
  </div>
  ...
  methods: {
    ...
    async updateTask() {
      ...
      try{
        ...
        if (this.editedTask.title === null || this.editedTask.title === '') {
          throw new Error('missing title');
        }
        if (this.editedTask.description === null || this.editedTask.description === '') {
